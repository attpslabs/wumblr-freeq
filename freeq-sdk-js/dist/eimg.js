 ...
 */
export async function deriveGroupKey(channel, members, epoch = 0) {
    ...
    for (const d of sorted) {
        if (deduped.length === 0 || deduped[deduped.length - 1] !== d)
            deduped.push(d);
    }
    ...
    const salt = new Uint8Array(await crypto.subtle.digest('SHA-256', enc.encode(channel.toLowerCase())));
...
export async function decryptBytes(key, blob) {
    if (blob.length < 12)
        throw new Error('eimg: blob too short to contain a nonce');
 ...
 */
export async function uploadEncryptedImage(origin, did, channel, members, contentType, imageBytes, epoch = 0) {
    const key = await deriveGroupKey(channel, members, epoch);
    const ciphertext = await encryptBytes(key, imageBytes);
    const form = new FormData();
    form.append('file', new Blob([ciphertext], { type: contentType }), 'eimg');
    form.append('did', did);
 ...
 */
export async function fetchEncryptedImage(origin, imageId, did, channel, members, epoch = 0) {
